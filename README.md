...
| **The three-sieve pair desert**: positivity ($U{+}V \le m^2$) + coherence (A9.3) + **class representation** (each line's co-norm triple must be represented by one even class at an admissible discriminant — strictly beyond genus) kill **every** ordered congrua pair for **every** center $m \le 10^4$ (2026-08-28 extension: 32,850 pairs at 1667 centers — 28,028/3,816/**1,006**, zero golden centers; previously 1782 pairs to $m \le 1200$; the $U{+}V$ diagonal unrepresentability and never-kill-a-real-line controls stand) — the A3 desert's first structural explanation at the pair level, now with a 1,006-pair anatomy corpus | PROVEN (necessity of each sieve) + VERIFIED($m \le 10^4$) | A9 §3; `a9.desert_ext` |
