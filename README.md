...
| **The three-sieve pair desert**: positivity ($U{+}V \le m^2$) + coherence (A9.3) + **class representation** (each line's co-norm triple must be represented by one even class at an admissible discriminant — strictly beyond genus) kill **every** ordered congrua pair for **every** center $m \le 1200$ (1782 pairs: 1608/152/22, zero remain; the $U{+}V$ diagonal is unrepresentable in all 22 character-passing pairs; never-kill-a-real-line controls built in) — the A3 desert's first structural explanation at the pair level | PROVEN (necessity of each sieve) + VERIFIED($m \le 1200$) | A9 §3 |
