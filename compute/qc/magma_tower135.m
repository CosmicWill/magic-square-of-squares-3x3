// Entry 135: the symmetry tower of the 28 surviving (1,1,1) classes (joint-sign quotient x = g^2, y = g h; then the
// double-inversion quotient u = x + 1/x, v = y + 1/y where the curve allows it).  Four genus-2 endpoints (classes 2918-2921),
// sixteen genus-4 second quotients (the (8,8) classes) and eight genus-4 joint quotients without a further symmetry
// (classes 2914-2917, 2940-2943).  Paste into the Magma calculator (120 s limit) one block at a time.
// A rational point on a quotient counts only if it lifts: u^2 - 4 and v^2 - 4 rational squares, x a rational square,
// and the frame ratios g, h admissible (not 0, +-1, oo); the Python side does the lift check from the point list.
P<uu,vv> := PolynomialRing(Rationals(), 2);
A := AffineSpace(P);

// ---- block 1: the four genus-2 second quotients: hyperelliptic model, rank bounds, torsion, Chabauty / points
print "class 2918";
...
b, H := IsHyperelliptic(C); print b; print H;
...
if ru eq 1 then G, m, rp, gp := MordellWeilGroupGenus2(J); print G, rp, gp; free := [G.i : i in [1..Ngens(G)] | Order(G.i) eq 0]; pts := Chabauty(m(free[1])); print "Chabauty", pts; end if;
...
// ---- block 2: the sixteen genus-4 second quotients: automorphism group, hyperellipticity
print "class 1632";
...
F := FunctionField(C); print Genus(F);
G := AutomorphismGroup(F); print "automorphisms", #G;
print "hyperelliptic", IsHyperelliptic(F);
...
Q<x,y> := PolynomialRing(Rationals(), 2); B := AffineSpace(Q);
print "class 2914";
