...
## 2026-09-08 — Entry 138: THE (1,1,1) BOX IS DEAD — the last sixteen classes excluded on their other frames through one genus-2 curve K (Magma: rank 1, the Mordell-Weil group proved, Chabauty); the symmetry tower on every frame; the fiber lifting

THE CURVE K. Frame 0 of each of the sixteen surviving (1,1,1) classes
carries the (8,8) component over the genus-3 curve H of entries 136-137
(Jacobian 666d1 x P, automorphism group of order 4, no odd-degree model:
out of reach of Chabauty). Frames 1 and 2 -- the engine and the towers
used one frame per class, but every frame is a necessary condition --
carry instead a single (4,4) component whose quotient by h -> -1/h is an
even octic in g, and the odd companion of that octic (w^2 = z Q(z),
z = g^2, w = g y; the module's new "odd" endpoint) is ONE curve for all
sixteen classes and both frames:
    K: w^2 = z^5 + 80 z^4 + 126 z^3 - 16 z^2 + z
(eight classes meet its reciprocal model z^5 - 16 z^4 + 126 z^3 + 80 z^2
+ z, the same curve under z -> 1/z, w -> w/z^3). Its Jacobian has the
Frobenius quartics of the Prym P. PARI: K = z x (an irreducible quartic),
so J(K)[2](Q) = Z/2 generated by (0,0) - oo; the point counts #J(F_p),
p <= 43, bound the torsion by 2; minimal discriminant -2^23 3^3 37 (the
odd part of the conductor is 999 = 3^3 37; PARI cannot fix the conductor
at 2 -- lfuncheckfeq fails -- so its analytic rank is not used); no
point of height <= 10^5 but (0,0). Magma (compute/qc/magma_tower138.m,
output magma_tower138.out.txt, run by the user): rank bounds 1 1
(2-Selmer group Z/2 + Z/2), torsion Z/2, MordellWeilGroupGenus2: J(K)(Q)
= Z/2 + Z with "rank proved: true, group proved: true"; Chabauty on the
free generator: K(Q) = {(0,0), oo} (the returned set {5} conditions only
the index of the argument's subgroup, which is 1 for a generator of the
proved group). Both points have z in {0, oo}, i.e. g in {0, oo}:
degenerate.

THE LIFT. The module composes the quotient map explicitly -- on these
components the endpoint variable is z = g^2 -- and pulls K's points back
as FIBERS: over z = 0 the rational points of the component with g = 0
(three per component: (0, -1), (0, 0), (0, 1)), over z = oo none.
No admissible pair. Every one of the sixteen classes has both frame 1
and frame 2 dead (the single component killed, no live univariate
factor); the base loci of the sixteen are empty (entry 104: Groebner
basis 1), so the exclusion is an exclusion of squares, not only of frame
pairs. Tally: dead 2944 / finite 0 / unknown 0.

THE (1,1,1) BOX IS DEAD. Every one of the 2,944 classes of labels and
signs with three distinct first-power split primes is excluded: 1,376 by
the engine (entries 97-103), the rest by the genus bounds, resolution,
common factors, quotient towers, the free-frame reduction, the
prime-column lemma, the height system, the bielliptic and genus-2
towers, the binomial square root, and the symmetry tower (entries
104-138); the finiteness statement of entry 104 (finitely many squares
up to scaling by the inert cofactor) becomes an exclusion: NO 3x3 MAGIC
SQUARE OF NINE DISTINCT SQUARES HAS A CENTER WHOSE SPLIT PART IS A
PRODUCT OF THREE DISTINCT FIRST-POWER PRIMES (Theorem A3.PQR, doc 2.62).
External inputs, recorded verbatim in compute/qc: Magma's Chabauty for
C_a, C_b (entry 132) and K (this entry), the LMFDB's rank 0 for C_c
(entry 131); everything else is PARI (2-descent rank bounds and torsion
of the elliptic endpoints), Sage (the bielliptic quadratic Chabauty of
G_1 and F_1, entries 118/130) and sympy. The claim is exactly the
ledger's: every class carries its mechanism and a check re-derives it or
a sample of it.

THE FIBER LIFTING (a correction). The module's lift of an endpoint's
points solved each step's relations one unknown at a time, for FINITE
endpoint points only, on the assumption that a point at infinity of the
endpoint forces a frame ratio in {0, oo}. That is true for the joint,
inversion and even quotients but FALSE after a coordinate change or a
reciprocal quotient: 1/U = oo at U = 0, i.e. a^2 = -s, and
w = o + kappa/o = oo at o = 0, which for o = Y = g h means g h = 0 --
degenerate here, but not by the stated argument. The module now composes
the forward maps of the steps (X = a^2, Y = a b; U = a + s/a; the
monomial changes; o; z = o^2; w = o + kappa/o) into the endpoint
variable as an explicit rational function N(g,h)/D(g,h) and takes the
rational points of {Phi = 0, N - v D = 0} over every endpoint value v
(the complete point set) and of {Phi = 0, D = 0} over the point at
infinity, by a resultant and rational roots; a shared component (an
infinite fiber) refuses the kill. Every one of the 110 kills of entries
136-137 was re-derived under the fibers: the recorded endpoint still
kills, 598 fiber points in all (the new ones beyond the step solver's are
the points with a coordinate in {0, +-1}, e.g. (+-1, 0), (0, +-1)),
none admissible; the sixteen survivors of the engine frame are still
not killed there. The routes gained the direct model (a component
already quadratic in one variable) and the odd companion.

THE SWEEP OF EVERY FRAME. compute/symmetry_tower.py on all three frames
of every open class, with the engine's own verdict (decide_component)
for every component: a class dies when some frame has every component
dead or killed and no live univariate factor. (1,1,1): 16 of 16 dead
(frames 1 and 2, K). (2,1,1): 102 of 242 dead (frame 0: 64, frame 1: 34, frame 2: 4) -- by endpoint: inversion h (s=-1) -> odd | K (28); double inversion (-1,-1) -> odd | K (16); direct | F1 (8); direct -> odd | C_c (8); direct | G1 (8); double inversion (-1,-1) -> reciprocal | rank-0 elliptic, torsion 4 (8); inversion g (s=-1) | G1 (8); inversion g (s=-1) -> odd | C_a (4); direct -> odd | C_a (4); direct -> odd | C_b (4); inversion g (s=-1) -> odd | K (4); direct -> reciprocal | F1 (2);
tally dead 79228 / finite 140. (3,1,1): 64 of 220 dead (frame 0: 64) -- by endpoint:
inversion h (s=-1) -> odd | K (28); direct | F1 (8); direct -> odd | C_c (8); direct | G1 (8); direct -> odd | C_a (4); direct -> odd | C_b (4); inversion g (s=-1) -> odd | K (4); tally dead 288727 / finite 156 (1,181 provisional). The stray
classical-tower job on the shape survivors promised in entry 137 was
stopped unfinished: the symmetry tower's routes contain the classical
quotients (the even and reciprocal quotients of the hyperelliptic
models), so a separate fold would add nothing. The open shape classes
sit, on every frame, over endpoints of positive rank or unidentified
genus-2 curves: (2,1,1) degree 8 / rank n/a / unidentified: 120, degree 16 / rank n/a / unidentified: 80, degree 9 / rank n/a / unidentified: 80, degree 6 / rank n/a / unidentified: 80, degree 12 / rank n/a / unidentified: 48, degree 7 / rank n/a / unidentified: 48; (3,1,1) degree 12 / rank n/a / unidentified: 328, degree 10 / rank n/a / unidentified: 152, degree 6 / rank n/a / unidentified: 144, degree 24 / rank n/a / unidentified: 136, degree 13 / rank n/a / unidentified: 136, degree 8 / rank n/a / unidentified: 128, degree 11 / rank n/a / unidentified: 92, degree 20 / rank n/a / unidentified: 72.

Data: compute/data_symmetry_tower_frames.json (the revalidation records,
K, every class's three frames with components, engine verdicts, kills,
endpoints and fiber points; module sha256 755c0c9dd545e666); the ledgers carry the
kills (verdict_before_symmetry_tower_frames / stf, stfb) and the block
symmetry_tower_frames. Check a3.symmetry_tower_frames (suite 220); entry
137's re-derivation accepts the fiber-based module. Memory and ROADMAP
(M15-L, R.15) updated: the (1,1,1) campaign is over; the atlas has K;
the next steps are the recurring positive-rank endpoints of the shapes
(Chabauty through Magma on the genus-2 ones, the (2,2,1) and (2,2,2)
boxes with the frames sweep from the start) and the trinomial cofactor.
Prepared during this entry: compute/qc/magma_tower139.m, ten Magma
blocks for the genus-2 endpoint curves that the open shape classes share
across their frames (eleven distinct curves among the classes swept
before the extraction; the most frequent met by sixteen classes; two with
