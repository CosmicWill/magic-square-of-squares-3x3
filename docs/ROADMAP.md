...
## R.15 The state of the program after entry 135 (2026-09-08): the arc, the theorems, the lessons, the paths

*Written at the user's request as a breather after entries 130–135, alongside the independent review's R.14.  It records how the work evolved, what stands, what the evolution teaches, and where the value is.  The existence question remains open.*

**Update after entry 138 (2026-09-08).**  The $(1,1,1)$ campaign is over: the box is dead (2,944 / 0; Theorem A3.PQR, doc 2.62) — the last sixteen classes through the genus-2 curve $K$ on their other frames, with Magma's Chabauty.  The frontier after entry 138: $(1,1,1)$ **dead**; $(2,1,1)$ 79228 / **140** open; $(3,1,1)$ **156** open (1,181 provisional); four primes 5,956 / 1,131; five primes 26,444 / 18,438.  After entry 139 (five more curves decided by Magma, the shapes re-swept): $(2,1,1)$ **132** open, $(3,1,1)$ **140** open.  After entry 140 (the twisted endpoints): $(2,1,1)$ **132** open, $(3,1,1)$ **132** open.  After entry 141 ($D_{140}$ decided): $(2,1,1)$ **132** open, $(3,1,1)$ **124** open.  After entry 142 (the Prym test): tallies unchanged; sixteen of the 88 route-less classes are proven beyond every quotient route through curves of genus $\le 2$ (their components' Jacobians have no factor of dimension $\le 2$), leaving for them Chabauty–Coleman on genus-3/4 quotients or the theory path.  After entry 143 (the census, R.16): 256 open = 24 genus-0 + 16 rank-1 elliptic + 32 rank-≥2 genus-2 + 40 genus-3 + 24 genus-4 + 32 genus-5 + 88 without endpoint; five hyperelliptic curves of genus 3–4 cover the 64 genus-3/4 classes (the Magma programme `magma_tower143.m`).  After entry 144 ($R_3$ decided through its genus-2 companion): $(2,1,1)$ **124** open, $(3,1,1)$ **124** open.  After entry 145 (the second-level quotient census): tallies unchanged; sixteen classes reach genus-2 curves at level 3, $R_5$'s closed by rank, the U family's $U_1$, $U_2$ undecided (Selmer bound 4).  Two lessons join the list below: every frame is a necessary condition and the second frame often dies where the first survives (sweep all frames from the start); and a lift must handle the points at infinity — the fiber of the composed map, not the step solver, is the right object.

### The arc in five movements

1. **Sieves and the Lucas grid** (A9, entries to ~50).  Every magic square of squares is a Lucas grid: a centre $m^2$ and offsets $U,V\in D(m)$ with $U\pm V\in D(m)$.  The pair sieves leak (Lemma A9.13: verdicts only soften under scaling; golden pairs are abundant), and the additive layer is bone-dry to $10^7$: the whole difficulty is the additive quadruple, and that layer is exact, not another sieve.
2. **The additive layer by hand and by machine** (A3.6–A3.10, entries 80–96).  No signed additive relation in $D(m)$ for one split prime (any power), for $p^2q$, $p^3q$, $p^2q^2$; the machine theorems (unit collapse, the window finisher, the deep descent) closed the boxes end to end; the $\omega\le2$ frontier is Conjecture $R_J$, a genus-$(J-1)$ curve over $\mathbb Q(i)$ per $J$.
3. **The $\omega=3$ curve engine** (entries 97–119).  Classes of labels and signs are plane curves; the $(1,1,1)$ box closed to finiteness (2,944 classes: dead uniformly or Faltings-finite); the minor formula (every quadruple curve a pullback of the circle); quotient towers and the fifteen killing curves; the singular-locus and base-locus theorems; the finiteness statements for shapes $(1,1,1)$ and $(2,1,1)$; the twist audit; the bielliptic descent of $\mathcal C_2$.
4. **The uniform local theory** (entries 120–129, 134).  The prime-column lemma A3.PC (valuations), the height system A3.HS with the reality sharpening (units and sizes), the orientation theorem A3.OR and T1 (what the rows are), the pair search (rigidity), and the binomial square root A3.SQ (the cofactor's factorisation).  Each is uniform in the number of primes and the shape, and each killed most of what was open when it arrived — but the kill rate of the local method falls with the number of primes (77% → 56% → 36% at three, four, five frames before A3.SQ).
5. **Curve arithmetic by symmetry** (entries 130–135).  The towers' bielliptic models, the general bielliptic test, the LMFDB and the Magma calculator as working tools, and now the symmetry tower (joint sign change, double inversion) that carries the $(1,1,1)$ survivors to four genus-$2$ and twenty-four genus-$4$ curves.

### What stands (uniform in the number of primes unless stated)

| Result | Statement | Where |
|---|---|---|
| A3.PC | the maximal absolute exponent of a prime column is attained by at least three labels; no free frame | 2.49 |
| A3.HS, T′, v3 | unit congruences → exact divisibilities → prime inequalities with exact certificates | A11 |
| A3.OR, T1 | the rows are functions of the flip pattern; three never-disagreeing offsets kill | A12 |
| A3.SQ | the binomial integer is a product of coprime factors of size $\sqrt P$: the deficient prime's exponent doubles | 2.58 |
| A3.6–A3.10 | no relation for one split prime, $p^2q$, $p^3q$, $p^2q^2$ (small exponents, $\omega\le2$) | 2.x |
| finiteness | shapes $(1,1,1)$, $(2,1,1)$: finitely many squares up to scaling (Faltings, ineffective) | entries 104, 113 |

The frontier after entry 134: $(1,1,1)$ 2,916 dead / **28** open; $(2,1,1)$ 79,072 / 296; $(3,1,1)$ 264 open (1,181 provisional); four primes 5,956 / 1,131; five primes 26,444 / 18,438.

### What the evolution teaches

- **Every uniform theorem came from keeping structure the previous method discarded.**  Valuations (A3.PC) discarded units; the height system kept the units' congruences but discarded the cofactors' shapes; A3.SQ kept one shape (a square) and doubled the exponent.  The cofactor of the trinomial row is the structure still discarded.
- **Local theorems kill in rounds of about 80% and decay with $\omega$.**  Boxes give evidence and templates, never the proof; each new prime adds a column at which orientations can flip, and flips are generic.
- **Curve arithmetic is reusable, and the same curves recur.**  Fifteen killers, then $G_1$ again, then 123a1, 88a1, 92b1, 664a1, 352b1/c1, 1840d1/184b1: the quotient constructions do not see the primes, only the class's combinatorics, which is why a curve decided for one class decides others.
- **The review loop pays.**  The twist audit, the prime-column lemma, the verifier gap and the joint-sign quotient all came from an independent reading; the certificate discipline (exact certificates, negative tests, gates in a clean worktree) is what makes a corpus this size trustworthy.

### Where the value is (ranked)

1. **The trinomial cofactor.**  Every $(1,1,1)$ survivor and most survivors elsewhere have two four-maxima columns, where only trinomial rows exist: $S=\sum a_XV_X^2$, a sum of three squares of primitive Gaussian integers, $\equiv0$ mod $\pi^{4M}$, with $S_1\bar G$ real (T′).  A3.SQ's sibling would be a structural fact about $S$ (a factorisation, a parity beyond version 3, a bound on $S_1$ from the reality condition's exact form).  Moderate probability; the largest payoff — it applies to every campaign at once.
2. **The symmetry tower to the first complete shape.**  Four genus-$2$ endpoints (Magma: rank bounds, Chabauty), twenty-four genus-$4$ curves (automorphisms first).  Closing $(1,1,1)$ entirely is plausible; it is the template for every shape.
3. **The atlas question.**  Run the tower on the $(2,1,1)$ and $(3,1,1)$ survivors.  If the endpoints are the same few curves — the minor formula suggests they should be, since the quotients depend on the class's combinatorics and not on the exponents' size — then "every shape" reduces to finitely many curves plus a uniform statement about which classes reach which curve.  That would be a global mechanism, the first candidate for goal G that is not a descent.
4. **The twisted-configuration descent** (path 3): the peeling identity gives one step and re-inserts the prime; recorded, low probability, revisited only if the cofactor work produces a new exact identity.
5. **Not now:** more boxes; effective bounds of abc type; the (2,1,1) point arithmetic before the atlas question is answered.

### The immediate sequence

*(Entry 136, the same day: the symmetry groups of the components are larger than the joint sign change — order 8 for the (8,8) components — and their elliptic quotients 80a1 and 528j2 have rank 0: twelve of the 28 dead without Magma; the last sixteen sit over 666d1 of rank 1 through genus-3 curves; the (2,1,1) survivors show the same tower with 42 elliptic and 42 genus-2 endpoints.  The atlas question has its first positive answer.)*

Fold the Magma results for `magma_tower136.m` (the E₃ curves; lifts checked in Python); run the symmetry tower on the $(2,1,1)$ and $(3,1,1)$ survivors (the atlas test); then the trinomial cofactor as the theory front, with the survivors' rows as the test set; keep the review loop.
...
**The surface.**  $V\subset\mathbb P^8$, six quadrics, general type, $K^2 = 576$, $c_2 = 768$, $p_g = 111$, $q = 0$, 256 nodes; algebraically quasi-hyperbolic (BTVA, A7 §7); $518\le\rho\le 544$ claimed by Auel–Singer (arXiv:2609.09351, September 2026) — entry 148: an exact principal minor of the archived matrix violates Hodge index, so that matrix does not establish the lower bound; the proposed bounds 456 and 454 require unproved rank/motivic identifications (A3 §2.70).  Bombieri–Lang predicts finitely many non-degenerate squares; nothing known proves it.  No map to a curve of positive genus; not a product-quotient; the geometric Bombieri–Lang results (varieties with finite maps to abelian varieties) do not reach it.
...
2. **The Shioda–Inose atlas** — explain the fifteen killer curves by the elliptic curves attached to the K3 pieces of $V$ (Picard rank 17–20 ⇒ Shioda–Inose structure); testable now with the L-polynomial pipeline (entry 142).  A positive answer is a structural theorem about the tower's reach and a uniform prediction of which classes no quotient route can ever decide.  **Outcome (entry 147, A3 §2.69): the strong form is false** — every K3 piece has good reduction outside $\{2,3\}$, so its Shioda–Inose curve over $\mathbb Q$ has conductor $2^a3^b$, and eleven of the fifteen killers do not; for the 22 pieces with good reduction at 3 the attached curves are exactly 128a (a twist of the killer 128c2), 32a (the Legendre killer $\lambda = 2$) and 256a; the other pieces have CM by $\mathbb Q(\sqrt{-3})$ and $\mathbb Q(\sqrt{-6})$ or are attached to $\mathbb Q$-curves and to representations induced from quadratic fields.  Thirteen of Auel–Singer's fourteen octic K3 types are identified exactly with modular objects (their Remark 4.10; entry 147 and its addenda: the level-96 and level-384 $\mathbb Q$-curve forms for orbits 7/14, 16 and 10, 12, 1, elliptic curves over $\mathbb Q(\sqrt 3)$ and $\mathbb Q(\sqrt 6)$ for orbits 8 and 15; orbit 9 awaits a Hilbert modular form over $\mathbb Q(\sqrt 2)$); the branch is closed as a route to the killers and stays open only as K3 arithmetic.
3. **The étale Brauer–Manin programme** (Auel–Singer's proposal) — the only visible route to an unconditional proof.  Contributions within reach: the Galois-module structure of the Picard lattice (their 1204 divisors, our 84 + 9 character atlas), $H^1(\mathrm{Gal},\mathrm{Pic})$, the local evaluations (F5).  Needs their paper digested in full and a licensed Magma for their scripts.  **Entry 148 (A3 §2.70):** 335 algebraic dimensions with Galois characters are obtained independently of the candidate K3/Horikawa identifications. The proposed 423-dimensional module and bounds $\rho\le456,454$ remain conditional on those identifications. An exact, source-verified five-row principal minor disproves the published matrix's interpretation as a divisor Gram matrix; the certificate replays offline. Next: correct the geometric intersection calculations, construct an integral Galois-stable lattice, and certify the rank transfers before computing $H^1(\mathbb Q,\mathrm{Pic})$.  **The barrier (entry 149, the proof-alternatives branch, `docs/research/global-obstructions.md` GB.3 and `finite-descent-barrier.md` FD.2, CITED from Harari and Demeio):** every finite subgroup of $\mathrm{Br}(U)$ and every finite torsor tower followed by finite Brauer tests leaves $U(\mathbb A_{\mathbb Q})$ non-empty with positive real component — the Brauer group computed here can constrain but cannot finish; an obstruction needs the full étale–Brauer set, a justified integral restriction of denominators, or a descent with a genuine decrease, and the branch's corner-label descents (AD, ND) fail with certificates.  **Atlas status (entry 148 addendum):** the two unidentified Horikawa types are not sums of atlas pieces and weight-3 newforms of level $2^a3^b \le 1152$ with Hecke field of degree $\le 2$ (`a3.horikawa_w3_148`); their Frobenius factors split over prime-dependent fields, so the next candidates are Asai pieces of Hilbert modular forms over $\mathbb Q(\sqrt2), \mathbb Q(\sqrt3), \mathbb Q(\sqrt6)$ — the class of the open orbit 9 — which the LMFDB Hilbert-form data would decide.
...
- *Uniform theorems, every ω and every shape:* the prime-column lemma A3.PC (entry 120), the height system A3.HS (entry 123), the orientation reading A3.OR with Theorem T1 (entry 129), the binomial square root A3.SQ (entry 134).  They say what any solution must look like and exclude most classes of every box; they stop at the orientation-generic classes with a feasible prime hierarchy.
- *Closed shapes:* one split prime for every exponent (Theorem A3.6, entries 80–96); at two split primes the split parts $p^aq^b$ with $(a,b)\in\{(1,1),(2,1),(2,2),(3,1),(3,2),(4,1),(4,2),(5,1),(3,3)\}$ and transposes (the quadruple engine, entries 95–96), an infinite remainder of exponent shapes never run and the additive Conjecture R_J open beyond them; at ω = 3 the $(1,1,1)$ box is dead — **Theorem A3.PQR** (entry 138): no magic square of nine distinct squares has a centre whose split part is a product of three distinct first-power primes.
- *The $(2,1,1)$ and $(3,1,1)$ boxes:* 79236 / **132** and 288759 / **124** (+1,181 provisional); the finiteness statement for $(2,1,1)$ (entry 113); the symmetry tower with its atlas — fifteen rank-0 elliptic curves and seven genus-2 curves decided by Chabauty ($K$, $B/C/G/H/J_{139}$, $D_{140}$) — recurring across shapes (entries 135–141).
- *Entry 142:* sixteen route-less classes have components whose Jacobians carry no factor of dimension ≤ 2: no quotient route through any curve of genus ≤ 2 can decide them.

**The open set: 256 classes, and what each waits on (entry 143, `compute/data_open_census_143.json`).**

| best endpoint genus | $(2,1,1)$ | $(3,1,1)$ | what would decide it |
|---|---|---|---|
| 0 (rational curve) | — | 24 | nothing on that frame; the other frames are worse |
| 1, rank 1 (three elliptic curves) | — | 16 | nothing by finiteness; an argument on the elliptic curve's points |
| 2, rank ≥ 2 ($A_{139}$ 16; $D_{139}$ 8; $I_{139}$ 8) | 16 | 16 | quadratic Chabauty if the rank is 2 (research code); nothing if 3 |
| 3 (R1, R2, R3) | 40 | — | 2-descent rank bound + Chabauty–Coleman (licensed Magma + the Balakrishnan–Tuitman package); R3's genus-2 companion on the calculator |
| 4 (R4, R5) | — | 24 | the same; R5's two genus-2 pieces on the calculator |
| 5 | — | 32 | the same, harder |
| none (route-less) | 76 | 12 | the theory path; 16 are proven beyond genus ≤ 2 |

**Outcome of the programme (entry 144).**  R3 decided through its genus-2 companion (rank 0, the group proved): its eight classes dead.  R1, R2 (rank ≥ 3 = genus), R4, R5 (rank ≥ 4 = genus) closed by rank, no hidden involutions; G2 rank 3, G3 rank 2; the analytic ranks timed out; $I_{139}\cong D_{139}$ (equal Euler factors, equal Igusa–Clebsch invariants), so its rank is 3.  The calculator's curve arithmetic on the shape survivors is exhausted: 248 open = 124 + 124.  **Entry 145** iterated the classical quotients on every endpoint model: sixteen $(3,1,1)$ classes gain a genus-2 curve at level 3 -- $R_5$'s eight closed by rank ($T_5=(z+4)G_2$, rank 2), eight more through the new curves $U_1$, $U_2$ (rank $\ge$ genus, closed).

**The paths, re-read (R.13).**  Path 2 (closing shapes by curve arithmetic) has a concrete next programme: the eleven blocks of `magma_tower143.m`, then Coleman integration on R1–R5 and on the genus-3/4 full quotients of entry 142 once ranks are bounded — a licence buys exactly this.  Path 3 (descent on the number of primes, the trinomial cofactor) is the theory front; the 88 route-less classes are its natural targets, and the branches `research/independent-cofactors` and `research/proof-alternatives` (the canonical finite-descent barrier, divisor reciprocity, the genus-129 fibres of nonlinear descent) are working there.  Paths 4 and 6 unchanged.  A new computational experiment with no Magma at all: the $(2,2,1)$, $(4,1,1)$ and $(2,2,2)$ boxes through the full pipeline (lemma, height system, engine, tower on all frames), to see whether the atlas keeps recurring and whether the residue of each box keeps the same structure.

**Tools.**  Sage and PARI do everything that is a genus, a quotient, a point count or an elliptic rank; the Magma calculator is reserved for genus-2 descent and Chabauty; a licensed Magma (about three thousand dollars for two years, or a university licence) would add 2-descent on higher-genus Jacobians and the Coleman-integration packages.  The LMFDB is out of range for every blocking curve.

**Honest assessment.**  Every box closed so far closed by the same means and left the same kind of residue: classes whose best curve has positive rank or genus above 2.  The proportion of such classes grows with ω (path 5), so the computational programme cannot end the problem on its own; it produces the theorems about what a solution must look like and the templates the theory path needs.  The recommendation, in order: run the eleven blocks; pursue the theory path on the route-less classes; mechanise the second-level quotient if G1–G3 succeed; start the next boxes as machine time allows.
...
| 3 | **Descent on the number of primes** (goal G) | a solution with support S forces one with smaller support, or a contradiction | the free-frame reduction did it for one family (now subsumed); the pair-search rigidity (the largest prime is determined by the others) is rigidity, not descent; **entry 134: the hierarchical classes' rigid rows led to Theorem A3.SQ (the binomial square root), which kills most open classes of every campaign; the peeling identity gives a one-step descent to a twisted configuration on N − 1 primes, and no further** | the trinomial rows (most survivors have two four-maxima columns); the twisted configurations as the object of a descent | the only arithmetic route to a full proof; low probability, but the first uniform theorem came from here |
| 4 | **The surface geometry** (see also the integrated research notes of entry 149: `docs/research/`, the GB.7 root-sum cover with no rational boundary points and unknown interior) | elliptic fibrations of the magic-square surface, Picard rank, Brauer classes; why the same fifteen rank-0 curves kill everything at genus 1 | fifteen killers across every route; the differential web; the foliation probe (PROOF-DIRECTIONS §5) | explain the killers by a fibration; test the foliation for a first integral | high; exploratory; unlikely to give full nonexistence alone (degenerate points are dense) |
...
**Entry 150 (2026-09-17): the web made explicit.** In the chart $u = 1$ the direction quartic of $\eta_\star$ is $\prod \ell_{(a,b)}$ times $Q = 3cv^2\,dc^4 - 2v(3c^2 + v^2 - 1)\,dc^3dv + 3c(c^2 + v^2 - 1)\,dc^2dv^2 + c(1 - c^2)\,dv^4$; absolutely irreducible with monodromy $S_4$ (no splitting into foliations over any cover); branch curve the binodal quartic $(v^2 + c^2 - 1)^2 = 4c^2(c^2 - 1)$ of genus 1 with nodes at $D_\pm$; top form $(v\,dc - c\,dv)^2\cdot$quadratic, so the top-degree Darboux condition is Euler-vacuous; the Darboux criterion $g \mid Q(c,v; g_v, -g_c)$ reproduces the fifteen integral lines (`a8.eta_star_web`, A8 §12). Carnicer-type bounds are ruled out; the wall lives on the direction surface $S$ with its tautological foliation. First actions updated: M12-C through the Darboux systems (six parameters per configuration); the extactic first-integral test on $S$; Brunella's index formulas at the eight triple points. **Entry 151: Lemma T** (`a8.web_tangency`): every branch of an integral curve through a triple point is tangent there to an integral line through that point (four choices at the $A/B$-points, five at the $D$-points); the cubic campaign is thereby a set of three-parameter homogeneous Darboux systems, $4{,}280$ sub-cases before symmetry, instead of six-parameter systems that Singular could not finish. **Entry 152: Theorem A8.20 — the cubic level is closed** (`a8.web_cubics_152`): parametrizing the rational cubic with the three triple points at $t = 0, 1, \infty$ makes every condition linear and the integrality identity a 24-equation system in two unknowns; over $\mathbb Q(\sqrt3)$ all 597 representatives have only maps onto lines (26 empty families). A2.C is now a statement about images of degree $\ge 4$; the same engine applies at every degree with the same 597 representatives. **Quartic level (entry 152 addendum):** modular evidence at one prime that no rational curve has a quartic image; the exact certificate stalls on coefficient growth (decomposition, emptiness certificate and modular lift all recorded as failed attempts) — closure at degree 4 needs structure that cuts the family below dimension 4.
...
| M15-S | 🏆 **2026-09-16 (entry 145) — THE SECOND-LEVEL QUOTIENT CENSUS: sixteen classes gain a genus-2 curve; U₁, U₂ undecided.** The classical quotients iterated on every endpoint model of the 248 open classes: R₅'s eight classes closed by rank (G₂, G₃, T₅ of ranks 3, 2, 2) and eight more reaching the new curves U₁, U₂ (2-Selmer bound 4, rank ≥ 1, undecided; a rank bound of 1 would kill all eight). The module chains the quotients to depth 4. Tallies unchanged. `a3.second_level_census_145`; suite 227. | W10 |
| M15-R | 🏆 **2026-09-15 (entry 144) — THE GENUS-3 ENDPOINT R₃ DECIDED: 8 of the 8 (2,1,1) classes that meet it dead.** R₃ is even in x; its genus-2 companion has rank 0 with the Mordell–Weil group proved (Magma block G1 of `magma_tower143.m`), hence exactly three points, and R₃ has only its two points at infinity. The tower's known curves extended to genus 3 (`KNOWN_HYPERELLIPTIC`, the matcher generalised to any even degree). Frontier: (2,1,1) 124, (3,1,1) 124 open. `a3.symmetry_tower_frames_144`; suite 226. | W10 |
| M15-Q | 🏆 **2026-09-15 (entry 143) — THE CENSUS OF THE OPEN CLASSES.** For each of the 256 open classes the best endpoint genus reachable by a whole frame (entry-140 records): genus 0: 24, rank-1 elliptic: 16, rank ≥ 2 genus-2: 32, genus 3: 40, genus 4: 24, genus 5: 32, none: 88. FIVE hyperelliptic curves of genus 3–4 cover the 64 genus-3/4 classes; R3 and R5 are even and split into genus-1/2 pieces. The Magma programme `magma_tower143.m` (11 blocks) prepared; the review R.16 written. Tallies unchanged. `a3.open_census_143`; suite 225. | W10 |
| M15-P | 🏆 **2026-09-15 (entry 142) — THE PRYM TEST: sixteen of the 88 route-less (2,1,1) classes proven beyond every quotient route through genus ≤ 2.** Their (7,3)/(8,4) components fall into four families sharing one full quotient by the (Z/2)² sign-and-inversion group; Magma: the full quotients are non-hyperelliptic (genus 3/4) with simple Jacobians for A, B; Sage (function fields over 𝔽_p, `compute/qc/prym_test.sage`): the three Prym parts of every family and the full quotients of C, D have irreducible L-polynomials at a good prime, so J(Γ) has no factor of dimension ≤ 2 — no map over ℚ to any curve of genus 1 or 2. The L-polynomial work left the Magma calculator; PARI shows the five open genus-2 curves outside the LMFDB's range. Tallies unchanged. `a3.prym_test_142`; suite 224. | W10 |
| M15-O | 🏆 **2026-09-09 (entry 141) — THE CURVE D₁₄₀ DECIDED: 8 of the 8 (3,1,1) classes that meet it dead.** The three new twisted endpoints share one quartic (D = xQ, E = (x+4)Q, F = x(x+4)Q); D has rank 1 and Chabauty's point set {(0,0), ∞}, certified by the 3-divisibility test of its generator (`magma_tower140b.m`); E and F have rank bounds 1–2 and no Richelot splitting, and stay open with the rank-2/3 curves. The classes meeting D were known exactly from the entry-140 records and re-run with D₁₄₀ known. Frontier: (2,1,1) 132, (3,1,1) 124 open. `a3.symmetry_tower_frames_141`; suite 223. | W10 |
| M15-N | 🏆 **2026-09-08 (entry 140) — THE TWISTED ENDPOINTS: the admissibility condition (g² + 1, h² + 1 squares) and the fiber discriminants of the quotient steps as quadratic twists of the tower's models; (2,1,1) 0 of 132 and (3,1,1) 8 of 140 open classes dead.** A Mordell–Weil sieve on rank-1 endpoints cannot certify (the degenerate points survive it); the twists turn the same conditions into new curves decided by PARI and the known genus-2 curves — the atlas recurs through them (a twisted odd companion is the reciprocal model of C_a). Every kill re-derived with the audited module; two corners of the reduction closed after an independent review. Frontier: (2,1,1) 132, (3,1,1) 132 open; `magma_tower140.m` lists the twisted genus-2 endpoints for the next Magma run. `a3.symmetry_tower_frames_140`; suite 222. | W10 |
| M15-M | 🏆 **2026-09-08 (entry 139) — FIVE MORE CURVES DECIDED BY MAGMA, THE SHAPES RE-SWEPT: (2,1,1) 8 of 140 and (3,1,1) 16 of 156 open classes dead.** The genus-2 endpoints shared by the open shape classes (`magma_tower139.m`, ten blocks run by the user): B₁₃₉, C₁₃₉, G₁₃₉, H₁₃₉, J₁₃₉ decided (rank ≤ 1, Mordell–Weil group proved or its index certified prime to 3 by `magma_tower139b.m`, Chabauty; two have only their points at infinity), five of rank 2–3 open. The module's known curves now number eleven; every frame of every open class re-swept. Frontier: (2,1,1) 132, (3,1,1) 140 open. `a3.symmetry_tower_frames_139`; suite 221. | W10 |
| M15-L | 🏆 **2026-09-08 (entry 138) — THE (1,1,1) BOX IS DEAD: all 2,944 classes excluded; Theorem A3.PQR (no magic square of squares has a centre whose split part is a product of three distinct first-power primes).** The last sixteen classes die on frames 1 and 2 through one genus-2 curve K: w² = z⁵ + 80z⁴ + 126z³ − 16z² + z (Magma: rank 1, J(K)(Q) = Z/2 + Z proved, Chabauty: K(Q) = {(0,0), ∞}, both degenerate). The module's lift corrected (fibers of the composed quotient map, the point at infinity included; all 110 earlier kills stand), the direct and odd-companion endpoints added, and every frame of every open class swept: (2,1,1) 102 of 242 dead, (3,1,1) 64 of 220 dead. `a3.symmetry_tower_frames`; suite 220. | W10 |
| M15-K | 🏆 **2026-09-08 (entry 137) — THE SYMMETRY TOWER ON THE SHAPE SURVIVORS: 54 of 296 open (2,1,1) and 44 of 264 open (3,1,1) classes dead.** `compute/symmetry_tower.py` (symmetry groups, quotient steps, coordinate changes, PARI endpoints, known genus-2 curves, exact lifts) reproduces entry 136 on the (1,1,1) box and kills through G₁, F₁, C_a, C_b, C_c and the 528j2-type reciprocal quotients: the atlas recurs across shapes. Tallies (2,1,1) 79,126 / 242, (3,1,1) 220 open. `a3.symmetry_tower_shapes`; suite 219. | W10 |
| M15-J | 🏆 **2026-09-08 (entry 136) — THE SYMMETRY TOWER KILLS: twelve of the 28 surviving (1,1,1) classes dead.** Every surviving component carries a group of sign-and-inversion maps of order 8 or 4; for classes 2918–2921 the quotient by (g,h) ↦ (−1/g, h), (g, −1/h) is a (2,2) curve with Jacobian 80a1 (rank 0), and for the eight order-4 classes the quotient by g ↦ −1/g is one hyperelliptic genus-3 curve whose quotient by h ↦ 1/h is 528j2 (rank 0); in both cases every rational point has a degenerate frame ratio. Tally dead 2928 / finite 16; the last sixteen (all (8,8)) sit over 666d1 (rank 1) through genus-3 curves E₃ (`magma_tower136.m`). The (2,1,1) tower reaches 42 elliptic and 42 genus-2 endpoints: the atlas recurs. `a3.symmetry_tower`; suite 218. | W10 |
| M15-I | 🧭 **2026-09-08 (entry 135) — THE REVIEW INTEGRATED, THE VERIFIER CLOSED, THE SYMMETRY TOWER; R.15.** The independent review's findings verified and folded (its eight target records dead by A3.SQ); `verify_certificate` now recomputes D, the exact product and the cap and rejects tampered or incomplete records (`a3.height_verifier`: six negative tests, 52 + 16 capped searches tied to their exact bounds); every one of the 28 surviving (1,1,1) classes is invariant under the joint sign change, and twenty also under the double inversion: quotients of genus 10 / 4, second quotients of genus 4 / 2 — four genus-2 endpoints (2918–2921) and twenty-four genus-4 curves await the Magma calculator (`compute/qc/magma_tower135.m`; lifts checked in Python). R.15 records the state of the program and ranks the paths: the trinomial cofactor, the symmetry tower to the first complete shape, the atlas question. Suite 217. | W10 |
| M15-H | 🏆 **2026-09-08 (entry 134) — THE BINOMIAL SQUARE ROOT (Theorem A3.SQ): the height system's binomial rows double their exponent; 118 + 1,092 + 761 + 2,021 + 10,472 open classes dead across the five campaigns (R.13 path 3).** Path 3 began with the hierarchical open classes: `compute/hierarchy.py` (forced ratios, rigid rows, frame divisibilities, the peeling identity) found rigid rows at four and five frames, and one worked by hand (p₀ = (p₄⁴ + 1)/2 against p₀ ≤ 2p₄²) exposed the general fact: the binomial integer Im A or Re A has A = B² with B primitive, so it is a product of coprime factors of size √P and the deficient prime's power divides one of them — p_j^{4g} ≤ P, ≤ 2P, or p_j^{8g} ≤ 4P. Version 4 of the height system; exact certificates, each new (version 3 feasible). Tallies: (1,1,1) 2916 / 28, (2,1,1) 79,072 / 296, (3,1,1) 264 open, four frames 5,956 / 1,131, five frames 26,444 / 18,438. In the (1,1,1) box every survivor has two four-maxima columns, elsewhere most: the next uniform lever is a trinomial sharpening. The descent formulation (twisted configurations) is recorded; the naive descent stops after one step. `a3.square_root`; suite 215. | W10 |
...
| M12-C | ✅ **CLOSED 2026-09-18 (entry 152, Theorem A8.20: no rational curve with cubic Lucas image; Lemma T + the parametrized web campaign, 597 representatives exact over $\mathbb Q(\sqrt3)$)** — earlier: ⚠️ opened 2026-08-28, first slice closed — **Theorem A8.19 proven**: no eta*-integral GRAPH cubic through ≥ 3 triple points (thirteen incidence families, all eliminations Qbar-complete, every solution a known integral line). New rational curves with cubic image need genuine degree-≥2 terms in both coordinates; the general 6-parameter-per-configuration eliminations are the continuing campaign, machinery built (`a8.web_cubics`) | W1 |
