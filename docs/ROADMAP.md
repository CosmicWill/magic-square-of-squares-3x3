...
**P-A (theory, highest value) — the uniform ω = 3 finiteness theorem.**  Statement to aim at: *for every exponent shape (a,b,c), up to scaling there are finitely many MSS3 with split part p^a q^b r^c.*  By 2.41 the base-locus half is done; what remains is that every component of every class has genus ≥ 2 or is dead by a uniform mechanism.  Evidence: minimum genus 3 in both boxes, no genus-2 component anywhere.  Program: (1) a genus formula from the labels — the dichotomy theorem gives g = (a−1)(b−1) − Σ δ_P over the base points and the boundary, the base points are classified (2.40), so bound the δ_P from the classification and the multiplicities of the minors; (2) the genus-0/1 components: show they occur only in the label patterns where the monomial lemma or a rank-0 quotient kills them, uniformly.  Computation: a per-component table over both boxes (labels, bidegree, base-point census with multiplicities, exact genus) to conjecture the formula; the (3,1,1) box as the first test outside the data it was found on.  This would be the first theorem covering infinitely many shapes — still ineffective, but the right kind of statement.
...
**P-E (parked) — R_J and goal G.**  R_J needs a global idea (or Magma Chabauty per J, which does not reach uniformity).  For goal G (reduce ω ≥ 3 to lower ω), one concrete lemma to examine: classes in which a frame appears in exactly one label reduce, after eliminating that element, to a three-term relation in the other two frames — the ω = 2 ladder; if such classes occur, the ladder theorems kill them uniformly; if the enumeration never produces them, that is worth understanding.
