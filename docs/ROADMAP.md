...
| M13-H | 🏆 **THEOREM 2026-08-30 — A3.9 (the (3,1) box)** — ⚠️ *status revised 2026-09-02 (entries 87–88): 74 of the 78 distinct OPEN patterns are machine theorems; the four H2 X6-route rows rest on a hand proof that never treated content 3 (3 | S₁, half of all frames) — one open lemma, numerically empty to p < 20000; see M13-R*: no signed additive relation for split part p³q. The complete pattern space closed by the machine layers + eight named tree-families (G1–G4 lemmas, the pincer, parity/leg-window finishers; Fermat appearances 5–8). **Corollary: any MSS3 center's split part is p²q², p⁴q+, or has ≥ 3 distinct split primes.** The (2,2) box is one family from Theorem A3.10 (`a3.p3q_theorem`) | W10 |
