...
Fold the Magma results for `magma_tower135.m` (lifts checked in Python); run the symmetry tower on the $(2,1,1)$ and $(3,1,1)$ survivors (the atlas test); then the trinomial cofactor as the theory front, with the survivors' rows as the test set; keep the review loop.
