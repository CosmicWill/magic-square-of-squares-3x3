...
> **R.10 status (entry 111, 2026-09-06).** O1: the field-free cross-check built (entry 110); the rigorous pass over the 25,378 provisional (2,1,1) classes and the elimination base-locus pass over its 67,236 finite classes are running (entry 112 folds them: the finiteness statement for shape (2,1,1)). O3: the minor-map base locus classified over the whole (1,1,1) box (entries 110–111) and THE ELIMINATION BASE-LOCUS THEOREM proved (2.41): the base-locus condition is automatic except for the classes whose eliminated frame has one absolute exponent in all four labels — the finiteness statements now rest on Faltings-finiteness plus a finite trinomial computation. Next in O3: the coset lemma for the minor-map locus in general, and why ten killers. O2 (Sage) still pending the user's decision.
