...
|---|---|---|---|---|---|
| 1 | **The role-word theorem** (uniform local structure) | Prove, for every ω and shape, which label configurations survive A3.PC + A3.HS; the linear programmes depend only on combinatorial data (exponent differences, spreads) | the same families survive in all three boxes; no class with two A/B deficits in different columns | derive the survivor family from the exponent matrices; test on the (1,1,1,1) box | cheap; high; a theorem about what any solution must look like, not a proof |
| 2 | **Closing one shape** (the (1,1,1) box to zero open classes) | curve arithmetic on the 170 survivors: rank bounds + Chabauty on the genus-2 quotients of the 32 tower classes; elimination curves and bielliptic models (entry 118's pipeline) for the rest | 24/24 genus-0-quotient classes closed; the bielliptic pipeline works | Magma rank bounds for the 32 genus-2 quotients | high; moderate; the first complete shape, a template |
...
| 5 | **Higher ω with the local method only** (the (1,1,1,1) box) | the lemma and the height system work for any number of columns and need no engine | **done (entry 127): it weakens** — 55.5% of the lemma survivors killed at four frames against 76.8% at three; two A/B deficits no longer exclude | the (1,1,1,1,1) box for the trend (minutes); then path 1 in its new form | minutes; answered |
