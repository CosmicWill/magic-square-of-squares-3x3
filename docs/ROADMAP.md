...
2. **The 1267 high-bidegree components** ✅ (entry 102): smoothness fails
   (they are singular at the degenerate frame values) and the local sieve
   is vacuous, but a rigorous Riemann–Hurwitz lower bound on the genus
   (ramification ≥ fiber degree − sum of point multiplicities, computed
   exactly over the branch-value fields in PARI, with an absolute-
   irreducibility certificate) certifies 1224 of them Faltings-finite; 43
   remain (the bound lost at singular points; Newton–Puiseux would
   sharpen it).  Box tally: dead 1373, finite 1528, unknown 43.
