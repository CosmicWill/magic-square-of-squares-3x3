...
**Theorem A9.11 (the overlattice lemma, full; PROVEN).** *Let $M$ be
the lattice generated by a nondegenerate syzygy witness (rank 2,
even, $\det M = Nq^2$) at an admissible sphere stratum ($N = 3m'^2$,
$m'$ odd, all values $\equiv 2 \bmod 8$ — which holds throughout the
family since $m'$ and the entry roots are odd, forcing $8 \mid U$
and co-norms $\equiv 2 \bmod 8$). Then an even overlattice
$L \supseteq M$ with $\det L = N$ exists; in particular $L$
represents $w_1, w_2, w_3$.*

*Proof.* Work prime by prime and glue. An index-$p$ integral
overlattice step drops $v_p(\det)$ by exactly 2, and a non-maximal
lattice always admits one; so from $M_p$ there is a chain of
integral overlattices descending in steps of 2 to the maximal
valuation. It remains to check, per prime, that the target
$v_p(N)$ lies on this chain (same parity, $\ge$ the maximal's
valuation):
(i) *$p$ odd, $p \nmid N$:* $v_p(\det M) = 2v_p(q)$ is even and the
determinant class is a unit, so maximal lattices are unimodular
($v_p = 0$); stop the chain at $v_p = 0 = v_p(N)$.
(ii) *$p = 3$:* $v_3(\det M) = 1 + 2v_3(q)$ is odd; every lattice in
the space has odd $v_3$, the maximal has $v_3 = 1$; stop at
$1 = v_3(N)$.
(iii) *$p \mid m'$, $p \nmid 6$:* $v_p(\det M) = 2v_p(m') + 2v_p(q)$
is even, determinant class is a unit, maximal is unimodular; stop
the chain at $v_p = 2v_p(m') = v_p(N)$ (strictly before
maximality, so every step exists).
(iv) *$p = 2$:* if $q$ is odd there is nothing to do. If not:
since every value $w \equiv 2 \bmod 8$ and $N \equiv 3 \bmod 8$,
the quadratic space is $V_2 \cong \langle 2u, 6u\rangle$ with
$u \equiv 1 \bmod 4$, hence isometric to $2 \times$(the norm form
of the unramified quadratic extension) — **anisotropic**. In an
anisotropic 2-adic space the even-maximal lattice is *unique*
(the integral-norm lattice), here of odd determinant in the
$3$-class; it contains $M_2$, with index exactly $2^{v_2(q)}$ by
the determinant ratio. Glue the local choices; the result is even
(the 2-adic component is), positive definite, of determinant $N$,
and contains the witness vectors. $\blacksquare$

**Theorem A9.12 (= Conjecture A9.C4, both directions; PROVEN).**
*A line is representable by a single class if and only if its
elementary Diophantine system — three pair equations
$w_iw_j - t_{ij}^2 = Nk_{ij}^2$ plus the rank-2 syzygy — has a
nondegenerate integer witness at some admissible stratum.*
Necessity is Theorem A9.9; sufficiency is Theorem A9.10 ($q = 1$)
extended by Theorem A9.11 (all $q$). *(Degenerate corner, flagged:
witnesses with all $k_{ij} = 0$ — all pairwise products perfect
squares — certify only via the single-value criterion and do not
occur in the data.)* **The representation sieve — hence the fourth
sieve, proven in M12-B to lie beyond every congruence and character
condition — is exactly an elementary Diophantine law.** The class
group was the language, never the mechanism; machine-verified: the
syzygy test reproduces the sieve verdict on all 88 anatomy lines
(31 alive + 57 killed) and the corpus samples, including the
2-adic boundary instance $m = 1885$ line 4 ($q = 4$), with the
congruence hypotheses ($8 \mid U$; co-norms $\equiv 2 \bmod 8$)
verified across the corpus with zero exceptions.
