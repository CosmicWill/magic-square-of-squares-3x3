...
## 7. Version 4: the binomial square root (entry 134)

The binomial integer of §2 is $\operatorname{Im}A$ or $\operatorname{Re}A$ with $A=B^2$, $B=\prod_{k\ne j}\pi_k^{d_k}$ primitive, so it is a product of factors coprime up to $2$ of size $\le\sqrt P$ (resp. $\sqrt2\sqrt P$), and $p_j^{2g}$ divides one of them: **(B1) becomes $p_j^{4g}<P$, (B1′) $p_j^{4g}\le2P$, and $p_j^{8g}\le4P$ when every $d_k$ is even** (Theorem A3.SQ, [2.58](A3-simultaneous-congrua.md#258-the-binomial-square-root-theorem-a3sq-the-height-systems-binomial-rows-double-their-exponent-entry-133-r13-path-3)).  Re-deciding every open class: $(1,1,1)$ 118 of 146 infeasible, $(2,1,1)$ 1,092 of 1,388, $(3,1,1)$ 761 of 1,025, four frames 2,021 of 3,152, five frames 10,472 of 28,910; in the $(1,1,1)$ box every survivor has two four-maxima columns, elsewhere most.  Check `a3.square_root`.


## 8. The certificate verifier, closed (entry 135)

The independent review of 2026-09-08 found that `verify_certificate` checked a capped coordinate's multiplier identity $\sum_iy_ia_i=e_j$ but neither recomputed the recorded bound fields ($D$, the exact product, the cap) nor rejected a feasible verdict with an empty per-prime record, so a capped certificate was not tied to the range the exhaustive search covered.  Now $D$ and $\prod_iK_i^{Dy_i}$ are recomputed exactly from the multipliers and must equal the record, the cap must be the $D$-th root of the product, a feasible verdict must carry every prime, and `exact_cap` returns the integer bound $\lfloor\text{product}^{1/D}\rfloor$.  Check `a3.height_verifier`: six tampered certificates rejected; the $52$ $(2,1,1)$ capped certificates re-verify and their exact bounds are covered by the recorded search caps; the $16$ $(3,1,1)$ capped searches' bounds, recomputed live at version 3, are covered by theirs.  The reviewer's independent recomputation of all $204$ bounds had found the same: a verifier gap, not a wrong exclusion.
