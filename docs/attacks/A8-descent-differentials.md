...
## 12. The web made explicit (entry 150, 2026-09-17; `a8.eta_star_web`)

In the chart $u = 1$ the direction quartic of $\eta_\star = \eta_4$ is a constant times $\prod_{(a,b)} \ell_{(a,b)}$ times the **reduced quartic**
$$Q(c, v; dc, dv) \;=\; 3cv^2\, dc^4 \;-\; 2v(3c^2 + v^2 - 1)\, dc^3 dv \;+\; 3c(c^2 + v^2 - 1)\, dc^2 dv^2 \;+\; c(1 - c^2)\, dv^4,$$
of degree $3$ in $(c, v)$ with only odd-degree parts. Its top form is $(v\,dc - c\,dv)^2\,(3c\,dc^2 - 2v\,dc\,dv - c\,dv^2)$, the square of the $A_0$-pencil form, so by Euler's identity the top-degree part of the Darboux condition below is automatic; its low form is $dv\,(2v\,dc^3 - 3c\,dc^2 dv + c\,dv^3)$. The discriminant of $Q$ in $s = dc/dv$ is a constant times $c\,v^2 \prod \ell_{(a,b)}\, B$ with the **branch quartic**
$$B:\ (v^2 + c^2 - 1)^2 = 4c^2(c^2 - 1),$$
irreducible over $\mathbb Q$, $\mathbb Q(i)$, $\mathbb Q(\sqrt2)$, $\mathbb Q(\sqrt3)$, with exactly two ordinary nodes at $(c, v) = (0, \pm1)$ — the diagonal triple points $D_\pm$ — hence of geometric genus $1$. At $(c, v) = (-3/2, 5/4)$ the specialized quartic has Galois group $S_4$ (irreducible, irreducible resolvent cubic, non-square discriminant), so $\operatorname{Gal}(Q/\mathbb Q(c,v)) = S_4$; the discriminant contains every line to an odd power, so the geometric monodromy is a normal subgroup of $S_4$ not inside $A_4$, i.e. $S_4$: **the web is absolutely irreducible and splits into foliations over no cover** (a $2$-extension cannot kill a $3$-cycle), so the Carnicer / Cerveau–Lins Neto degree bounds for plane foliations do not apply to it directly. **Darboux criterion (PROVEN, mechanical):** $g(c,v) = 0$ is $\eta_\star$-integral iff $g \mid Q(c, v;\, g_v, -g_c)$; it reproduces the fifteen lines of Theorem A8.17 (thirteen in the chart, plus $v = 0$; $u = 0$ is the other chart) and rejects controls. The W1 wall is thereby restated: an integral curve of degree $d$ is a Darboux polynomial of an absolutely irreducible $4$-web with cofactor of degree $3d - 1$ and forced top form, equivalently an invariant curve of the tautological foliation on the $4$-fold direction cover $S \to \mathbb P^2$ branched along $B$; a degree bound must come from the singular locus (the eight triple points, where $\eta_\star$ vanishes to order $\ge 4$, and the pencil carriers), not from the degree at infinity. Next: M12-C through the Darboux systems (six parameters per triple-point configuration); the direction surface $S$, its foliation and the extactic first-integral test; Brunella's indices at the triple points.

## 13. Lemma T: tangency at the triple points (entry 151, 2026-09-18; `a8.web_tangency`)

**Lemma T (PROVEN).** Every analytic branch of an $\eta_\star$-integral curve through a triple point $P$ is tangent at $P$ to one of the integral lines of Theorem A8.17 through $P$: at the $A$- and $B$-points the three entry lines and the pencil carrier (four directions), at the $D$-points the three entry lines and the two $\sqrt3$-lines (five). *Proof.* $Q$ vanishes identically at each triple point, so for $\gamma(s) = P + s\,d + O(s^2)$ one has $Q(\gamma, \gamma') = s\,L_P(d) + O(s^2)$ with $L_P(d) = (d\cdot\nabla)Q(P; d)$, the direction variation contributing nothing since $Q(P;\cdot) \equiv 0$; cusps reduce to the same by homogeneity. In the chart $u = 1$, $L_P \propto dc\,dv^2(dc - dv)(dc + dv)$ at $A_0, A_\pm$ and $L_P \propto dc\,(dc - dv)(dc + dv)(3\,dc^2 - dv^2)$ at $D_\pm$; the $B$-points are reached through the homogeneous form $Q_{\mathrm{hom}} = u^3 Q(c/u, v/u;\cdot,\cdot)$ with $X = u\,dc - c\,du$, $Y = u\,dv - v\,du$: $u^2 \mid Q_{\mathrm{hom}}(c,u,v; X, Y)$ (regularity of $\eta_\star$ along $u = 0$, the meaning of the radial square in the top form) and the quotient at $v = 1$ is $Q(c, u; dc, du)$ exactly, so the $u \leftrightarrow v$ symmetry of the web holds on the nose and the $B$-points carry the $A$-type forms. $\blacksquare$

Along the entry line $c = 0$ the web has the line's own direction as a triple direction and $dv = 0$ as the fourth; along $v = 0$ the own direction is double and the transversal slopes are $dc/dv = \pm 1/\sqrt3$.

**Consequence for M12-C.** A rational curve with cubic image passes through $\ge 3$ triple points (A8.15) and is integral (A8.11); prescribing at three of them the tangent line among the four or five admissible ones leaves a linear system of cubics of projective dimension $3$, so the Darboux condition of §12 becomes a homogeneous system in four unknowns per sub-case ($4{,}280$ sub-cases over the 56 configurations, fewer up to $D_4$). Each is expected zero-dimensional, containing exactly the products of three integral lines through the configuration; any other solution would be a new integral cubic. The six-parameter systems (26 equations of degree 10) did not finish in Singular within an hour even modulo a prime. Lemma T applies at every degree; it does not by itself bound the degree.

## 14. Theorem A8.20: the cubic level is closed (entry 152, 2026-09-18; `a8.web_cubics_152`)

**Theorem A8.20 (PROVEN).** No irreducible rational cubic in the Lucas plane is $\eta_\star$-integral. Hence no complete curve of geometric genus $0$ on $X$ has a cubic Lucas image (A8.11), M12-C is closed, and Conjecture A2.C holds through degree $3$: a nonconstant $k(t)$-MSS3 in characteristic $0$ has Lucas image of degree $\ge 4$.

*Proof.* A birational parametrization $\varphi = (p:q:r): \mathbb P^1 \to C$ of degree $3$ passes through three distinct triple points (A8.15), moved to $t = 0, 1, \infty$; by Lemma T (§13) the branches there are tangent to integral lines through the points. Point and tangency conditions are linear in the twelve coefficients and in $\lambda_1, \lambda_\infty$ ($\lambda_0 = 1$); integrality is the identity $E(t) = T(p, q, r; p', q', r') = 0$ with $T$ the coordinate-free form of §13, whose coefficients generate an ideal $I$ in the family parameters. $T$ is $D_4$-invariant up to sign, so the $4{,}280$ sub-cases (three points, three admissible tangents) reduce to $597$ orbit representatives. For each, either the linear system is inconsistent ($26$), or every point of $V(I)$ is a map of rank $\le 2$ — each $3\times3$ minor of the coefficient matrix lies in $\sqrt I$ by Rabinowitsch, or $I = 0$ with the minors identically zero — ($571$; $904$ components). A rank-$\le 2$ map has image a line. Exact over $\mathbb Q(\sqrt3)$ (Sage/Singular, 75 s) and modulo $32003$ (23 s), pinned in `compute/data_web_cubics_152.json`; replayed in pure Python (`compute/web_cubics_param.py`) on six representatives in the fast profile and on all $597$ in the full profile. $\blacksquare$

Two earlier formulations are recorded: six-parameter implicit systems did not finish in Singular in an hour even modulo a prime; four-parameter implicit systems with prescribed tangents produced positive-dimensional spurious components, the products (integral line)$\cdot M^2$ with a moving line $M$, on which the divisibility test is vacuous — each proven non-reduced over its function field, every isolated solution a product of integral lines (partial). The parametrized formulation has no such artefact: a squared line is not a birational image. The same engine works at every degree $d$: the same $597$ representatives, a family of dimension $3d - 7$, $E$ of degree $9d - 4$. **Quartic level (entry 152 addendum, `a8.web_quartics_152b`): modular evidence only.** Over $\mathrm{GF}(32003)$ all 597 representatives at $d = 4$ have integrality loci consisting of maps of rank $\le 2$ and no rank-3 member (family dimension 5 in 593 and 6 in 4 representatives; 1950 components, 1927 of them proper subvarieties of their family and 23 the whole family, all consisting of maps of rank $\le$ 2; 0 rank-3 points; 0 errors). The exact certificates did not finish: decomposition over $\mathbb Q(\sqrt3)$ and over $\mathbb Q$, the emptiness certificate $1 \in I + (1 - y\,m\,\lambda_1\lambda_\infty)$, and the modular lift (cofactors of $272{,}951$ terms) all stalled on coefficient growth in the positive-dimensional integrality locus. An exact closure needs additional structure cutting the family below dimension 4, not more computing. FAILED-ATTEMPT recorded; the cubic level stands as the theorem.
