...
## 2.62 The $(1,1,1)$ box is dead: the curve $K$, the fiber lifting, the sweep of every frame (entry 138; `compute/symmetry_tower.py`)

*(2026-09-08; entry 138; `compute/data_symmetry_tower_frames.json`; `compute/qc/magma_tower138.m` and its output; check `a3.symmetry_tower_frames`.)*

**The curve $K$.**  On frames $1$ and $2$ every one of the sixteen surviving classes of 2.60–2.61 has a single $(4,4)$ component whose quotient by $h\mapsto -1/h$ is an even octic in $g$; the odd companion of that octic, $w^2 = zQ(z)$ with $z=g^2$, is one curve for all sixteen classes and both frames:
$$K:\quad w^2 = z^5 + 80z^4 + 126z^3 - 16z^2 + z$$
(eight classes meet the reciprocal model $z^5-16z^4+126z^3+80z^2+z$, the same curve under $z\mapsto 1/z$).  $K = z\cdot(\text{irreducible quartic})$, so $J(K)[2](\mathbb Q)=\mathbb Z/2$; the point counts bound the torsion by $2$; the minimal discriminant is $-2^{23}3^3\cdot 37$.  Magma (run by the user, output recorded verbatim): rank bounds $1,1$; $J(K)(\mathbb Q)\cong \mathbb Z/2\oplus\mathbb Z$ with the group proved; Chabauty on the generator: $K(\mathbb Q)=\{(0,0),\infty\}$.  Both points have $z\in\{0,\infty\}$, i.e. $g\in\{0,\infty\}$: every rational point of the component is degenerate, on frame $1$ and on frame $2$ independently.

**Theorem A3.PQR (the $(1,1,1)$ box).**  Every one of the $2{,}944$ classes of the $(1,1,1)$ box is dead: the ledger `compute/data_omega3_box111.json` reads dead $2944$ / finite $0$ / unknown $0$, every class carrying its mechanism (the engine and the box closed to finiteness, 2.33–2.34; the genus bounds, the resolution, the common factors, the quotient towers, the free-frame reduction, A3.PC, A3.HS, the bielliptic and genus-$2$ towers, A3.SQ, the symmetry tower) and a check re-deriving it or a sample of it.  With the elimination base-locus theorem (2.41) and the base-locus classification of entry 104 (2.34) (every finite class had an empty or degenerate base locus; the sixteen last ones an empty one), the finiteness statement of entry 104 becomes an exclusion: **no $3\times3$ magic square of nine distinct squares has a centre whose split part is a product of three distinct first-power primes.**  External inputs, recorded verbatim in `compute/qc`: Magma's Chabauty for $C_a$, $C_b$ (entry 132) and $K$ (entry 138), the LMFDB's rank $0$ for $C_c$ (entry 131); the elliptic endpoints are PARI's (2-descent rank bounds, torsion, points), $G_1$ and $F_1$ are first-party (bielliptic quadratic Chabauty, entries 118/130).

**The fiber lifting (a correction of 2.61).**  The step solver lifted finite endpoint points only, assuming a point at infinity of the endpoint forces a frame ratio in $\{0,\infty\}$ — false after a coordinate change or a reciprocal quotient ($1/U=\infty$ at $U=0$, i.e. $a^2=-s$; $w=o+\kappa/o=\infty$ at $o=0$).  The module now composes the steps' forward maps into the endpoint variable $v = N(g,h)/D(g,h)$ and takes the rational points of $\{\Phi=0,\ N-v_0D=0\}$ over every value $v_0$ of the complete point set and of $\{\Phi=0,\ D=0\}$ over infinity (a resultant and rational roots; a shared component refuses the kill).  All $110$ kills of entries 136–137 stand under the fibers (598 fiber points, none admissible).

**The sweep of every frame.**  Each frame is a necessary condition; the engine and the towers used one.  Running the module on all three frames of every open class, with the engine's own verdict for every component:

| Campaign | Open before | Dead | Open after | Dead frames | Endpoints of the kills |
|---|---:|---:|---:|---|---|
| $(1,1,1)$ | 16 | 16 | **0** | frame 1: 16 | inversion h (s=-1) -> odd: $K$ (32 = both frames of each class) |
| $(2,1,1)$ | 242 | 102 | 140 | frame 0: 64, frame 1: 34, frame 2: 4 | inversion h (s=-1) -> odd | K (28); double inversion (-1,-1) -> odd | K (16); direct | F1 (8); direct -> odd | C_c (8); direct | G1 (8); double inversion (-1,-1) -> reciprocal | rank-0 elliptic, torsion 4 (8); inversion g (s=-1) | G1 (8); inversion g (s=-1) -> odd | C_a (4); direct -> odd | C_a (4); direct -> odd | C_b (4); inversion g (s=-1) -> odd | K (4); direct -> reciprocal | F1 (2) |
| $(3,1,1)$ | 220 | 64 | 156 | frame 0: 64 | inversion h (s=-1) -> odd | K (28); direct | F1 (8); direct -> odd | C_c (8); direct | G1 (8); direct -> odd | C_a (4); direct -> odd | C_b (4); inversion g (s=-1) -> odd | K (4) |
...
## 2.66 The Prym test: sixteen route-less classes beyond every quotient through genus $\le 2$ (entry 142)

*(2026-09-15; entry 142; `compute/data_prym_test_142.json`; `compute/qc/prym_test.sage`; `compute/qc/magma_tower142.m`, `magma_tower142b.m` and their transcripts; check `a3.prym_test_142`.)*

Of the 88 open $(2,1,1)$ classes with no hyperelliptic route (2.64), the sixteen with the smallest components — one component $\Gamma$ of bidegree $(7,3)$ or $(8,4)$ on one frame — form four families of four classes; within a family the components are isomorphic over $\mathbb Q$ (the maps $(g,h)\mapsto(g,h)$, $(g,-1/h)$).  The sign-and-inversion group $G=\{1,\ \text{joint},\ \text{diag}(1,1),\ \text{diag}(-1,-1)\}\cong(\mathbb Z/2)^2$ acts on $\Gamma$; Magma finds the full quotients $\Gamma/G$ non-hyperelliptic (genus 3 plane quartics, genus 4 canonical curves).  By Kani–Rosen, $J(\Gamma)\times J(\Gamma/G)^2\sim\prod_{H} J(\Gamma/H)$ over the three index-2 subgroups and $J(\Gamma/H)\sim J(\Gamma/G)\times P_H$, so every simple factor of $J(\Gamma)$ is a factor of $J(\Gamma/G)$ or of a Prym part $P_H$.  The $L$-polynomials of these pieces (Sage function fields over $\mathbb F_p$: places of degree $\le\dim$, Newton's identities, the functional equation; the reductions keep their genus, so the Jacobians have good reduction and the polynomials are characteristic polynomials of Frobenius; validated against Magma and against independent full counts) have, at the witness primes, no irreducible factor of degree $\le 4$:
...
## 2.68 The second level of the classical quotients: the census, T5, and the curves U1, U2 (entry 145)

*(2026-09-16; entry 145; `compute/second_level_census.py`; `compute/data_second_level_census_145.json`; `compute/qc/magma_tower145.m` and its transcript; check `a3.second_level_census_145`.)*

The tower applied the classical quotients (even, odd companion, reciprocal, reciprocal twist) once per route model; $R_3$ fell to a second application (2.67).  Applied recursively to every endpoint model of every finite component of every live frame of the 248 open classes, the deeper levels give a genus $\le 2$ endpoint on a whole frame to sixteen $(3,1,1)$ classes at level 3: $R_5$'s eight through $G_2$ (rank 3), $G_3$ (rank 2) and $T_5:\ y^2=(z+4)\,G_2(z)$, the LMFDB curve 230224.a.920896.1 of rank 2 (Magma on the model: rank bounds 2 2), closed by rank; and eight more through two new curves $U_1$ ('even, then reciprocal') and $U_2$ ('reciprocal, then even') of conductor $5\cdot 13\cdot 1490039$.  Magma: one curve ($U_1\cong U_2$), 2-Selmer group $(\mathbb Z/2)^4$, no Richelot isogeny, rank between 1 and 4, undecided; a rank bound of 1 would kill all eight (the dry run).  The module now chains the quotients to depth 4 (`CLASSICAL_DEPTH`) with the twist roots of every level pulled back through their own composed maps.  Frontier: $(2,1,1)$ 124, $(3,1,1)$ 124 open.
...
## 2.69 The Shioda–Inose atlas test: the K3 pieces of $V$ identified with modular forms (entry 147)

*(2026-09-16; entry 147; `compute/data_shioda_inose_147.json`; check `a3.shioda_inose_147`; the scratch scripts are named in the entry.)*

**The pieces.**  On the Lucas plane with coordinates $(x,y,z)$ the nine entries are the lines $L_0 = z$, $L_1^\pm = z \pm x$, $L_2^\pm = z \pm y$, $L_3^\pm = z \pm (x+y)$, $L_4^\pm = z \pm (x-y)$ (eight triple points, no quadruple point).  For a 6-subset $S$ the double plane $Y_S\colon w^2 = \prod_{i\in S} L_i$ is a K3 surface with $15 - 3t_3$ nodes and $t_3 = t_3(S)$ points of type $D_4$; the hyperplane and the $15 + t_3$ exceptional curves are defined over $\mathbb Q$, so
$$u_p(S) := \#\widetilde Y_S(\mathbb F_p) - 1 - p^2 - p\,(16 + t_3)$$
is the trace of Frobenius on the remaining part $R$ of $H^2$, of rank $6 - t_3$, which contains the transcendental lattice.  The data file records $u_p$ for all 84 pieces and all primes $3 \le p \le 241$.  The pieces fall into 14 classes by their trace sequences, the $D_4$-orbits of 6-subsets with two pairs merged: Auel–Singer's fourteen magic octic K3 surfaces (their Proposition 4.11), identified orbit by orbit through point counts of their Table 2 models mod $p$ (their octic $X_S$ is the $(\mathbb Z/2)^5$-cover of the plane branched along the six lines and $Y_S$ is its quotient by the even sign changes, so the transcendental parts agree; their node count is $4t_3$).  Only their orbits 4 and 6 (both $t_3 = 3$, both $\rho = 20$) are matched as a pair.  Four triples of lines become concurrent mod 3; the 22 pieces containing none of them have good reduction at 3 and are exactly the classes A–D below.

**The identifications (exact at all 51 primes $5 \le p \le 241$).**

| class | pieces | $t_3$ | A–S orbit, $\rho$ | identity | reading |
|---|---|---|---|---|---|
| A | 8 | 3 | 11, 19 | $u_p = \chi_{-2}(p)\,(a_p(128a1)^2 - p)$ | $R = \operatorname{Sym}^2 H^1(128a)\otimes\chi_{-2}$; 128c2 (twist by $-1$, rank 0) is a quotient killer of entry 100 |
| B | 6 | 2 | 2/13, 20 | $u_p = (a_p(32a2)^2 - 2p)_{p\equiv 1\,(4)} + p\,(1 + \chi_{-1}(p))$ | CM by $\mathbb Q(i)$ through $y^2 = x^3 - x$ (Legendre $\lambda = 2$, a killer); two classes conjugate over $\mathbb Q(i)$ |
| C | 4 | 3 | 4 or 6, 20 | $u_p = (a_p(256a1)^2 - 2p)_{p\equiv 1,3\,(8)} + p\,\chi_{-2}(p)$ | CM by $\mathbb Q(\sqrt{-2})$ through 256a1 ($j = 8000$); one class over $\mathbb Q(\sqrt{-2})$ |
| D | 4 | 3 | 6 or 4, 20 | $u_p = (a_p(256a1)^2 - 2p)_{p\equiv 1,3\,(8)} + p$ | the same CM; one class over $\mathbb Q$ |
| E | 2 | 0 | 3 (Bremner), 20 | $u_p = (a_p(36a1)^2 - 2p)_{p\equiv 1\,(3)} + p\,(2 + \chi_{-1}(p) + \chi_3(p))$ | CM by $\mathbb Q(\sqrt{-3})$ through $y^2 = x^3 + 1$; four classes (two over $\mathbb Q$, one over $\mathbb Q(i)$, one over $\mathbb Q(\sqrt 3)$) |
| F | 4 | 1 | 5, 20 | $u_p = a_p(f) + p\,(2 + \chi_2(p))$ | $f$ the weight-3 newform of level 24, character $\chi_{-24}$, CM by $\mathbb Q(\sqrt{-6})$ ($a_5 = -2$, $a_7 = -10$, $a_{11} = 10$; class number 2, no elliptic curve); three classes (one over $\mathbb Q$, two over $\mathbb Q(\sqrt 2)$) |
| G | 12 | 2 | 7/14, 19 | $u_p = \chi_{-3}(p)\,(r_p - p) + p$ | $\operatorname{Sym}^2$ of the level-96 newform with nebentypus $\chi_{12}$ (a $\mathbb Q$-curve: quartic coefficient field, inner twists by $\chi_{-1}, \chi_3, \chi_{-3}$), $r_p = a_p^2/\chi_{12}(p)$; one class over $\mathbb Q$ (addendum; checked to 241) |
| H | 4 | 1 | 16, 19 | $u_p = \chi_{-3}(p)\,(r_p - p) + p\,(1 + \chi_{-3}(p))$ | the same form; two classes, over $\mathbb Q$ and over $\mathbb Q(\sqrt{-3})$ (addendum) |
| I | 8 | 2 | 8, 18 | $u_p = \chi_{-2}(p)\,\operatorname{tr}_p \operatorname{As}(E)$ | $E$ = 2.2.12.1-1024.1-a1 over $\mathbb Q(\sqrt 3)$: $y^2 = x^3 - (1+\sqrt 3)x^2 + 2x$, conductor norm 1024, no CM; the transcendental lattice of $\operatorname{Res}_{\mathbb Q(\sqrt 3)/\mathbb Q} E$; no algebraic class in $R$ (addendum; checked to 241) |
| J | 8 | 2 | 15, 18 | $u_p = \chi_{-2}(p)\,\operatorname{tr}_p \operatorname{As}(E)$ | $E$ = 2.2.24.1-128.1-a1 over $\mathbb Q(\sqrt 6)$: $y^2 = x^3 - (1+\sqrt 6)x^2 - 2x + (2\sqrt 6 + 2)$, conductor norm 128, no CM (addendum) |
| K | 8 | 2 | 10, 19 | $u_p = \chi_3(p)\,(r_p - p) + p$ | $\operatorname{Sym}^2$ of the level-384 newform with nebentypus $\chi_{24}$ (= the level-768 form with nebentypus $\chi_{12}$, coefficient field $\mathbb Q(\sqrt{-2})$, inner twist $\chi_3$); one class over $\mathbb Q$ (second addendum; checked to 241) |
| L | 4 | 2 | 12, 19 | $u_p = \chi_{-6}(p)\,(r_p - p) + \chi_{-2}(p)\,p$ | the same form; one class over $\mathbb Q(\sqrt{-2})$ (second addendum) |
| M | 4 | 1 | 1, 19 | $u_p = \chi_{-6}(p)\,(r_p - p) + p\,(1 + \chi_3(p))$ | the same form; two classes, over $\mathbb Q$ and over $\mathbb Q(\sqrt 3)$ (second addendum) |
...
**Entry 148 addendum (2026-09-17): the weight-3 newform test for the two open Horikawa types.** The rank-9 and rank-10 Horikawa trace types (eight-line subsets omitting one of $L_1^\pm, L_2^\pm$, resp. one of $L_3^\pm, L_4^\pm$) admit no decomposition into atlas pieces and weight-3 newforms of level $2^a 3^b \le 1152$ with odd quadratic character and Hecke field of degree $\le 2$ (252 forms archived in `compute/data_horikawa_w3_148.json`; the search over 156 candidate pieces and the eight twists $\pm1, \pm2, \pm3, \pm6$ is replayed by `a3.horikawa_w3_148`; the no-$L_0$ type reproduces its candidate $\mathrm{CM}(\sqrt{-3}) \oplus \mathrm{Sym}^2(f_{96})\otimes\chi_3 \oplus \mathrm{Sym}^2(f_{96})\otimes\chi_3$ as the positive control). The real quadratic Hecke fields in that range are exactly $\mathbb Q(\sqrt2), \mathbb Q(\sqrt3), \mathbb Q(\sqrt6)$, while the Frobenius factors of the open types split over prime-dependent fields ($\mathbb Q(\sqrt2)$ at $p = 5$ for the rank-9 type; $\mathbb Q(\sqrt5)$ and $\mathbb Q(\sqrt{13})$ at $p = 5$ for the rank-10 type; scratch, unpinned), so the remaining candidates are Asai pieces of Hilbert modular forms over $\mathbb Q(\sqrt2), \mathbb Q(\sqrt3), \mathbb Q(\sqrt6)$ or forms of higher level. The conditional bounds and their conditionality are unchanged; a trace match is a candidate and an empty finite search excludes nothing beyond its candidate set.
