...
The next target is a global compatibility condition that retains the
canonical 2-adic domain, or a further cover/descent eliminating the
remaining boundary. Any proposed Brauer class must survive all original
square roots and the four cycle roots; its ramification on a proper
model and its evaluations on the required local domain need an explicit
audit. An obstruction found solely on a quotient is insufficient.
