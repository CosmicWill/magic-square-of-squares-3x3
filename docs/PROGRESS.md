...
*Current checkpoint: 2026-09-06. The verification registry contains 212 checks; this is not a claim that each proof has a complete automated certificate replay. The [README ledger](../README.md#claims-ledger), [research log](../RESEARCH_LOG.md), and [current roadmap](ROADMAP.md#r12-current-research-portfolio-2026-09-06) give the detailed record.*

## Current arithmetic checkpoint

The magic-square existence question remains open. The prime-column lemma A3.PC is a uniform necessary condition for every exponent shape and every number of split primes. It eliminates every free-frame class. After the completed C2 bielliptic descent, the lemma's fold and the height system of entry 123 (Theorem A3.HS: the unit congruences as prime inequalities, decided by linear programming with exact certificates), the working ledgers contain 2,786 dead / 158 open finite classes for the full (1,1,1) box (entry 130: twelve tower classes closed by the bielliptic descent), 77,980 dead / 1,388 open finite classes for the additional (2,1,1) campaign (entry 124: the reality sharpening of the trinomial rows), and 287,858 dead / 1,025 open finite (1181 provisional, 0 undecided) for the additional (3,1,1) campaign (entry 126, the lemma and the height system applied before any curve). The open classes carry finiteness certificates only; the height-system kills are unconditional for every choice of primes.
