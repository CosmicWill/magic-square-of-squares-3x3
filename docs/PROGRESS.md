...
The [research inventory](RESEARCH-INVENTORY.md) contains the 272 certified finite records (0 + 132 + 140 after entry 139), with exact candidate and source provenance. It excludes provisional and higher-prime height-only records; the review lists the whole recorded frontier. The engine applies A3.PC before curve decisions. [A10](attacks/A10-cancellation-descent.md) proves that the relaxed Laurent system has no further valuation-support obstruction, and derives leading-square restrictions and unit congruences. A global descent from those congruences is still an open investigation.
