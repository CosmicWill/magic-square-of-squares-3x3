...
def research_inventory(ctx):
    ...
    require(Counter(r["shape"] for r in stored["classes"]) == {"211": 132, "311": 124})      # entry 138: the (1,1,1) box is dead; entries 139-141: more curves, the twisted endpoints
    require(len({r["id"] for r in stored["classes"]}) == 256)
    require(Counter(tuple(r["binomial_height_recession_direction"] or []) for r in stored["classes"]) ==
            {(1, 1, 1): 216, (1, 1, 2): 24, (1, 2, 2): 16})      # entries 123-141: the inventory rebased on the survivors of the three campaigns (entry 141: D140 leaves 132 + 124)
    require(sum(not r["binomial_constraints"] for r in stored["classes"]) == 0)      # entries 137-141: every no-binomial class is dead
