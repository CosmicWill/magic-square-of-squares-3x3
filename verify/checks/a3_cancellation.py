...
def research_inventory(ctx):
    ...
    require(Counter(r["shape"] for r in stored["classes"]) == {"211": 124, "311": 124})      # entry 138: the (1,1,1) box is dead; entries 139-144: more curves, the twisted endpoints, R3
    require(len({r["id"] for r in stored["classes"]}) == 248)
    require(Counter(tuple(r["binomial_height_recession_direction"] or []) for r in stored["classes"]) ==
            {(1, 1, 1): 208, (1, 1, 2): 24, (1, 2, 2): 16})      # entries 123-144: the inventory rebased on the survivors of the three campaigns (entry 144: R3 leaves 124 + 124)
    require(sum(not r["binomial_constraints"] for r in stored["classes"]) == 0)      # entries 137-144: every no-binomial class is dead
